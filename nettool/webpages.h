...
<body>
  ...
  <button onclick="OIButton()">Old</button>
  <button onclick="listFilesButton()">List File (1)</button>
  <button onclick="listFilesButton2()">List File (2)</button>
...
function OIButton() {
  document.getElementById("status").innerHTML = "old interface.";
  var xhr = new XMLHttpRequest();
  xhr.open("GET", "/OI", true);
  xhr.send();
  window.open("/OI","_self");
}
...
function listFilesButton2() {
  xmlhttp=new XMLHttpRequest();
  xmlhttp.open("GET", "/list?path=/", false);
  xmlhttp.send();
  document.getElementById("detailsheader").innerHTML = "<h3>Files<h3>";
  document.getElementById("details").innerHTML = xmlhttp.responseText;
}
