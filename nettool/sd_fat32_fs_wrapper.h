...
public:
    ...
    virtual fs::FileImplPtr open(const char* path, const char* mode, const bool create) {
        Serial.printf("FileImplPtr open aufgerufen %s\n",path);
...
*/  //        return std::make_shared<SdFatFile32Impl>(sd.open(path,O_WRONLY|O_CREAT));// _convert_access_mode_to_flag(mode, create)));
